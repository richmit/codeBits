...
#+BEGIN_SRC calc :results output verbatum :exports both :wrap "src text :eval never :tangle no"
deriv(3*x^2+log(x), x)
#+END_SRC
...
#+BEGIN_SRC maxima :results output verbatim :exports both :wrap "src text :eval never :tangle no"
programmode:false;
